...
const bypassTests = async (page) => {
    page.evaluateOnNewDocument(() => {
        ...
        let removeNavigatorItems = ["deviceMemory", "bluetooth", "connection", "doNotTrack", "hardwareConcurrency", "keyboard", "locks", "permissions", "presentation", "storage", "usb", "userActivation", "webkitPersistentStorage", "webkitTemporaryStorage"];

        removeNavigatorItems.forEach((item) => {
            window.navigator.__defineGetter__(item, function () {
                return undefined;
            });
        });
        ...
        window.navigator.requestMediaKeySystemAccess = async function () {
            throw new Error("NotSupportedError: The operation is not supported."); // It's not supported on ios Safari or Chrome
        };
    });
};
    ...
    await mainPage.evaluateOnNewDocument(() => {
        ...
        window.emal = function(code) {
            ...
            esprima.parseScript(code, { range: true }, function (node) {
                if (node.type === "BinaryExpression" && node.right.type === "MemberExpression" && node.left.type === "MemberExpression" && node.operator === "^") {
            ...
            esprima.parseScript(code, { range: true }, function (node) {
                if (node.operator === "+=") {
                    ...
                    nameArray.push({
                        index: node.range[0],
                        insertCode: `;window.varName.push(${rightSide});if (${rightSide}.includes('probab')){debugger;};`,
                    });
                }
            });
            ...
            //时间戳
            // if (code.includes('p5d[69]=b7S.d1o()[X4AA.y1o]')) {code = code.replace('p5d[69]=b7S.d1o()[X4AA.y1o]','debugger;p5d[69]=b7S.d1o()[X4AA.y1o]')}
...
exports.run = async function (inject) {
    ...
    let possibleBodies = [];
    for (let i = 0; i <= results.arrays.left.length - 48; i++) {
        possibleBodies.push([]);
        index = 0;
        for (let j = i; j < results.arrays.left.length; j = j + 4) {
            possibleBodies[i].push(results.arrays.left[j]);
            index++;
            if (index >= 12) {
                break;
            }
